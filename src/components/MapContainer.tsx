    ...
    routesToRender.forEach(route => {
      ...
      route.waypoints.forEach((wp, idx) => {
        ...
        const marker = L.marker([wp.lat, wp.lng], {
          icon: L.divIcon({
            html: `<i class="fas fa-bus" style="color: ${route.color}; font-size: ${isTerminal ? '20px' : '16px'};"></i>`,
            className: 'custom-bus-icon',
            iconSize: [isTerminal ? 20 : 16, isTerminal ? 20 : 16],
            iconAnchor: [isTerminal ? 10 : 8, isTerminal ? 10 : 8],
          }),
        }).addTo(layerGroup);
